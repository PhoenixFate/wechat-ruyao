...
module.exports = function(Chart) {
	...
	Chart.Legend = Chart.Element.extend({
		...
		fit: function() {
				...
				if (isHorizontal) {
					...
					helpers.each(me.legendItems, function(legendItem, i) {
						...
						if (lineWidths[lineWidths.length - 1] + width + labelOpts.padding >= me.width) {
							totalHeight += fontSize + (labelOpts.padding) + 10;
          ...
          var itemHeight = fontSize + vPadding + 10;
			...
			if (opts.display) {
				...
				var drawLegendBox = function(x, y, legendItem) {
					...
					if (opts.labels && opts.labels.usePointStyle) {
						...
						if (!isLineWidthZero) {
              ctx.strokeRect(x, y, boxWidth, fontSize + 10);
						}
            ctx.fillRect(x+15, y, boxWidth, fontSize + 10);
					}

					ctx.restore();
				};
				var fillText = function(x, y, legendItem, textWidth) {
					ctx.fillText(legendItem.text, boxWidth +  x - 39, y + 4);//todo 使legend向下偏移一点
